...
/**
  ...
  *              环形队列->不浪费一个位置
  */
object CircleArrayQueueUnit2 {
  def main(args: Array[String]): Unit = {
    // 初始化一个队列
    val queue = new CircleArrayQueue2(4)
    // 保存用户输入的指令
    var keyWord = ""
    while (true){
      println("show：显示队列")
...
class CircleArrayQueue2(ArrayMaxSize:Int){
  // 初始化队列的相关信息
  // 队列的大小
   ...
   */
  var front = 0;
  /*  指向队列的尾部，注意这里定义的是指向队列的最后一个进入的元素的位置（含）
   *  0     1     2     3
   *              rear
   *              指针
   *  添加前移动到当前位置
  ...
  // 判断队列满的方法
  def isFull():Boolean = {
    (rear + 1) % maxSize == (front-1)
  }

  // 判断队列空的方法
  def isEmpty():Boolean={
    // 由于出队列会写入0，因此可将0纳入判断条件
    ((front-1) == rear) || arr(rear) == 0
  }
